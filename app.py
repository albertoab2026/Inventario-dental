...
with t2: # STOCK CON BUSCADOR
    st.subheader("📦 Consulta de Almacén")
    bus_stock = st.text_input("🔍 Escriba el producto para filtrar:", key="bus_s").upper()
...
with t3: # REPORTES
    ...
    if movs_aud:
        st.warning(f"Se registraron {len(movs_aud)} cambios en stock hoy:"); st.dataframe(pd.DataFrame(movs_aud)[['Hora', 'Producto', 'Tipo']], use_container_width=True, hide_index=True)
...
with t5: # CARGAR
    ...
    with st.form("fn"):
        ...
        pc_n = st.number_input("PRECIO COSTO", min_value=0.0); pv_n = st.number_input("PRECIO VENTA", min_value=0.0)
        if st.form_submit_button("🚀 GUARDAR"):
            if pn:
                if not df_inv[df_inv['Producto'] == pn].empty:
                    st.error(f"❌ '{pn}' ya existe. Use 'MANT.' para reponer o cambiar precios.")
                else:
...
with t6: # MANTENIMIENTO MEJORADO
    ...
    if p_m:
        p_s = st.selectbox("Confirmar Selección:", p_m, key="psm")
        ...
        if op == "➕ REPONER STOCK":
            ...
            n_costo = col_p1.number_input("Nuevo Costo Compra:", value=float(r_m.Precio_Compra))
            n_venta = col_p2.number_input("Nuevo Precio Venta:", value=float(r_m.Precio))
            if st.button("💾 GUARDAR NUEVOS PRECIOS"):
                tabla_stock.update_item(
                    Key={'TenantID': st.session_state.tenant, 'Producto': p_s},
                    UpdateExpression="SET Precio_Compra = :pc, Precio = :pv",
                    ExpressionAttributeValues={':pc': str(n_costo), ':pv': str(n_venta)}
                )
                registrar_kardex(p_s, 0, f"CAMBIO PRECIOS: C:{n_costo} V:{n_venta}")
                st.success("✅ Precios actualizados correctamente"); import time; time.sleep(1); st.rerun()
