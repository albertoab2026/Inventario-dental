...
# 1. PESTAÑA DE VENTAS (CON CONFIRMACIÓN)
with tabs[0]:
    if st.session_state.boleta:
        st.balloons(); st.success("✅ VENTA REGISTRADA")
        ...
        ticket += f"""
            ...
            <div style="text-align: right; font-size: 17px;"><b>TOTAL NETO: S/ {float(b['total_neto']):.2f}</b></div>
            <hr style="border-top: 1px dashed black;">
        ...
        if st.button("➕ AÑADIR AL CARRITO", use_container_width=True) and prod_filt_v:
            if cant <= info['Stock']:
                ...
                st.toast(f"✅ Agregado: {p_sel}")
                st.session_state.reset_v += 1; time.sleep(0.3); st.rerun()
        ...
        if st.session_state.carrito:
            ...
            st.warning("⚠️ Verifique los datos antes de finalizar.")
            confirma = st.checkbox("¿Desea realmente procesar esta venta?")
            
            if st.button("🚀 FINALIZAR Y REGISTRAR VENTA", type="primary", use_container_width=True, disabled=not confirma):
...
with tabs[1]:
    ...
    if not df_f.empty:
        def color_stock(val): return 'color: #FF4B4B; font-weight: bold;' if val < 5 else ''
        st.dataframe(df_f.style.map(color_stock, subset=['Stock']).format({"Precio": "S/ {:.2f}", "P_Compra_U": "S/ {:.2f}", "Stock": "{:d}"}), use_container_width=True, hide_index=True)
...
# 4. HISTORIAL (REPORTES DE ELIMINADOS INCLUIDOS)
with tabs[3]:
    st.subheader("📋 Movimientos y Auditoría")
    f_hist = st.date_input("Fecha a consultar:", key="f_hist_k").strftime("%d/%m/%Y")
...
with tabs[4]:
    ...
    with st.form("f_cargar"):
        ...
        cant_c = c1.number_input("Cant:", min_value=1, value=1)
        if st.form_submit_button("📥 CARGAR"):
            if p_final:
                f, h, _, uid = obtener_tiempo_peru()
                n_total = int(df_stock[df_stock['Producto'] == p_final]['Stock'].values[0]) + cant_c
                tabla_stock.update_item(Key={'Producto': p_final}, UpdateExpression="set Stock = :s", ExpressionAttributeValues={':s': n_total})
                tabla_auditoria.put_item(Item={'ID_Ingreso': f"I-{uid}", 'Fecha': f, 'Hora': h, 'Producto': p_final, 'Cantidad_Entrante': int(cant_c), 'Stock_Resultante': int(n_total)})
                actualizar_stock_local(); st.rerun()

# 6. MANTENIMIENTO (CON REPORTE DE ELIMINACIÓN)
with tabs[5]:
    st.subheader("🛠️ Administración")
    p_edit = st.selectbox("Editar:", df_stock['Producto'].unique())
    if p_edit:
        inf = df_stock[df_stock['Producto'] == p_edit].iloc[0]
        col1, col2 = st.columns(2)
        nc = col1.number_input("Nuevo Costo:", value=float(inf['P_Compra_U']))
        nv = col2.number_input("Nuevo Precio:", value=float(inf['Precio']))
        if st.button("💾 Guardar Cambios"):
            tabla_stock.update_item(Key={'Producto': p_edit}, UpdateExpression="set P_Compra_U = :c, Precio = :p", ExpressionAttributeValues={':c': str(round(nc, 2)), ':p': str(round(nv, 2))})
            actualizar_stock_local(); st.rerun()
            
    st.divider()
    st.write("### 🗑️ Eliminar Producto")
    p_del = st.selectbox("Seleccionar para borrar:", [""] + df_stock['Producto'].tolist())
    if st.button("🗑️ ELIMINAR") and p_del != "":
        f, h, _, uid = obtener_tiempo_peru()
        # GUARDAMOS EL REPORTE DE ELIMINACIÓN EN AUDITORÍA
        tabla_auditoria.put_item(Item={
            'ID_Ingreso': f"DEL-{uid}", 
            'Fecha': f, 
            'Hora': h, 
            'Producto': f"❌ ELIMINADO: {p_del}", 
            'Cantidad_Entrante': 0, 
            'Stock_Resultante': 0
        })
        tabla_stock.delete_item(Key={'Producto': p_del})
        st.error(f"Producto {p_del} eliminado. Registro guardado en historial.")
        actualizar_stock_local(); time.sleep(1.5); st.rerun()
