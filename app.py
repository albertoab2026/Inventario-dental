...
# 1. Limpiamos el Stock para que sea número entero (sin .0)
if 'Stock_Actual' in df_vis.columns:
    df_vis['Stock_Actual'] = pd.to_numeric(df_vis['Stock_Actual']).astype(int)

# 2. Limpiamos el Precio para que tenga Soles y 2 decimales
...
    c1, c2 = st.columns(2)
    with c1:
        # Ordenamos la lista del selector también
...
        # Buscamos el producto por nombre de forma segura
...
            # Restamos el stock en la memoria usando el nombre del producto
...
        st.warning("No hay ventas hoy.")
 asi esta bien fijate o esta mal si est mal mandame el actualizado
