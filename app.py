...
# --- LOGIN OBLIGATORIO (Si no está autenticado, detiene el resto del código) ---
if not st.session_state.auth:
    ...
    st.stop() # DETIENE TODO AQUÍ SI NO HAY LOGIN
...
with t1:
    if st.session_state.boleta:
        ...
        st.markdown(f"""<div style="background-color:white;color:black;padding:20px;border:2px solid #333;max-width:350px;margin:auto;font-family:monospace;">
            ...
            <hr><div style="display:flex;justify-content:space-between;"><b>TOTAL NETO:</b><b>S/{b['total_neto']:g}</b></div>
        ...
        bus_v = st.text_input("🔍 Buscar:", key="bus_venta").upper()
        prod_v = [p for p in df_inv['Producto'].tolist() if bus_v in str(p)]
        c1, c2 = st.columns(2)
        p_sel = c1.selectbox("Producto:", prod_v, key="sel_venta") if prod_v else None
        cant = c2.number_input("Cant:", min_value=1, value=1, key="cant_venta")
        if p_sel:
            row = df_inv[df_inv['Producto'] == p_sel].iloc
        ...
        if st.session_state.carrito:
            ...
            m_pago = st.radio("Pago:", ["💵 EFECTIVO", "🟣 YAPE", "🔵 PLIN"], horizontal=True, key="pago_venta")
            total_b = df_c['Subtotal'].sum()
            rebaja = st.number_input("Rebaja:", min_value=0.0, value=0.0, key="rebaja_venta")
            total_n = max(0.0, total_b - rebaja)
            if st.button(f"🚀 COBRAR S/ {total_n:g}", use_container_width=True, key="fin_venta"):
                f, h, uid = obtener_tiempo_peru()
...
with t2:
    st.subheader("📦 Stock en Tiempo Real")
...
with t3:
    ...
    f_rep = st.date_input("Día:", datetime.now(tz_peru), key="f_reporte").strftime("%d/%m/%Y")
...
with t4:
    ...
    f_h = st.date_input("Fecha:", datetime.now(tz_peru), key="f_hist").strftime("%d/%m/%Y")
...
with t6:
    ...
    bus_m = st.text_input("🔍 Buscar para reponer:", key="bus_mant").upper()
    prod_m = [p for p in df_inv['Producto'].tolist() if bus_m in str(p)]
    if prod_m:
        p_m = st.selectbox("Confirmar:", prod_m, key="sel_mant")
        s_act = int(df_inv[df_inv['Producto'] == p_m]['Stock'].values)
        c_mas = st.number_input("¿Cuánto entra?", min_value=1, key="cant_mant")
        if st.button("✅ REPONER", use_container_width=True):
            n_t = s_act + c_mas
            tabla_stock.update_item(Key={'TenantID': st.session_state.tenant, 'Producto': p_m}, UpdateExpression="SET Stock = :s", ExpressionAttributeValues={':s': n_t})
