...
with t2: # STOCK
    ...
    bus_stock = st.text_input("🔍 Filtrar Stock por nombre:", key="bus_s").upper()
...
with t3: # REPORTES
    st.subheader("📊 Ganancia Neta del Día")
...
with t4: # HISTORIAL
    ...
    f_h = st.date_input("Fecha de consulta:", datetime.now(tz_peru), key="fh").strftime("%d/%m/%Y")
...
with t6: # MANTENIMIENTO
    ...
    if p_m:
        ...
        if op == "➕ REPONER STOCK":
            ...
            n_c = c_p1.number_input("Nuevo Costo:", value=float(r_m.Precio_Compra))
            n_v = c_p2.number_input("Nueva Venta:", value=float(r_m.Precio))
            if st.button("💾 GUARDAR PRECIOS"):
                ...
                registrar_kardex(p_s, 0, f"AJUSTE PRECIOS: C:{n_c} V:{n_v}")
