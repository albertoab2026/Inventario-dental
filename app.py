...
# --- TAB 1: VENTA (CON CÁLCULO EN VIVO) ---
with tabs[0]:
    if st.session_state.boleta:
        ...
        <div style="background-color: white; color: black; padding: 20px; border: 2px solid black; border-radius: 10px; font-family: monospace;">
            ...
            <p>FECHA: {b['fecha']} {b['hora']}</p><hr>
            {"<br>".join([f"{i['Cantidad']} x {i['Producto']} - S/ {i['Subtotal']:.2f}" for i in b['items']])}
            <hr><h3 style='text-align:right;'>TOTAL: S/ {b['total']:.2f}</h3>
        </div>
        ...
        if not df_stock.empty:
            ...
            nota = st.text_input("Nota (ej. Rebaja)", key=f"n_{p_sel}")
...
# --- TAB 4: HISTORIAL (LIMPIO Y EN ESPAÑOL) ---
with tabs[3]:
    ...
    if h_data:
        ...
        # Ordenar por fecha y hora para ver lo último arriba
