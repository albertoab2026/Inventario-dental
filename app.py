...
TABLA_MOVS = 'SaaS_Movimientos_Test' # Nueva tabla para el historial de stock
...
with t1:
    if st.session_state.boleta:
        ...
        st.markdown(f"""<div style="background-color:white;color:black;padding:20px;border:2px solid #333;max-width:350px;margin:auto;font-family:monospace;">
            ...
            <p style="text-align:center;border:1px solid #ccc;">PAGO: {b['metodo']}</p></div>""", unsafe_allow_html=True)
        ...
        if p_sel:
            ...
            st.info(f"Precio: S/ {row['Precio']:.2f} | Stock: {row['Stock']}")
            if st.button("➕ Añadir"):
                if cant <= row['Stock']:
        ...
        if st.session_state.carrito:
            ...
            st.markdown(f"## Total: S/ {total_n:.2f}")
...
with t2:
    st.subheader("📦 Almacén Actual")
    bus_s = st.text_input("🔍 Filtro Stock:").upper()
...
with t3: # REPORTES DE VENTAS
    ...
    f_sel = st.date_input("📅 Fecha:", datetime.now(tz_peru)).strftime("%d/%m/%Y")
    ...
    if v_data:
        ...
        st.metric("GANANCIA TOTAL", f"S/ {df_v['Utilidad'].sum():.2f}")
...
with t4: # HISTORIAL DE MOVIMIENTOS (KARDEX)
    ...
    f_h = st.date_input("📅 Fecha Historial:", datetime.now(tz_peru), key="f_h").strftime("%d/%m/%Y")
    ...
    else: st.info("No hay ingresos ni ajustes hoy.")
with t5:
    st.subheader("📥 Registro de Entrada")
    with st.form("f_carga"):
        p_n = st.text_input("PRODUCTO").upper()
        s_n = st.number_input("CANTIDAD ENTRANTE", min_value=0)
...
with t6:
    ...
    if not df_inv.empty:
        ...
        ns = st.number_input("Nuevo Stock Físico:", value=0)
