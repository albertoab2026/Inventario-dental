...
def cargar_datos():
    try:
        ...
        if not df.empty:
            # Limpiamos los números para que no salgan muchos ceros
...
if not df.empty:
    ...
    # Mostramos el precio con solo 2 decimales
...
if st.session_state.carrito:
    ...
    if cf.button("🚀 FINALIZAR VENTA", type="primary", use_container_width=True):
        ...
        # Quitamos el bloque try/except que causaba el mensaje rojo falso
        ...
        st.balloons() # ¡Aquí están los globitos!
        st.success(f"✅ ¡Venta en {metodo_pago} guardada exitosamente!")
        st.session_state.carrito = []
...
with st.expander("🔐 PANEL DE ADMINISTRADOR"):
    if not st.session_state.admin_auth:
        ...
        c_abast = st.number_input("Cantidad:", min_value=1, value=5, key="cant_admin")
                ...
                for v in ventas:
                    ...
                    for p in v.get('Productos', []):
                        filas.append({
                            ...
                            "TOTAL": float(v['Total']) if primera else ""
                        })
                        primera = False
                    filas.append({k: "" for k in ["Fecha", "Producto", "Cant", "P. Unit", "Pago", "TOTAL"]})
                
                df_ex = pd.DataFrame(filas)
                out = io.BytesIO()
                with pd.ExcelWriter(out, engine='xlsxwriter') as writer:
                    df_ex.to_excel(writer, index=False)
                st.download_button("📥 Bajar Excel", out.getvalue(), "Reporte.xlsx")
