...
with t1:
    if st.session_state.boleta:
        ...
        ct = c2.number_input("Cant:", min_value=1, value=1, key=f"cv_{p_s}") # Key dinámica para resetear
        if p_s:
            ...
            if st.session_state.confirmar:
                ...
                if cc1.button("✅ SÍ"):
                    ...
                    for i, it in enumerate(st.session_state.carrito):
                        ...
                        n_s = int(df_inv[df_inv['Producto']==it['Producto']]['Stock'].values) - it['Cantidad']
...
with t5:
    ...
    with st.form("fn"):
        ...
        if st.form_submit_button("🚀 GUARDAR"):
            ...
            st.success(f"✅ ¡{pn} GUARDADO EXITOSAMENTE!"); import time; time.sleep(1); st.rerun()
...
with t6:
    ...
    if p_m:
        ...
        if op == "➕ REPONER STOCK":
            # Key dinámica basada en el producto para que se resetee al cambiar de ítem
            ...
            if st.button("✅ ACTUALIZAR STOCK", use_container_width=True):
                ...
                st.success(f"✅ ¡STOCK DE {p_s} ACTUALIZADO A {n_t}!"); import time; time.sleep(1); st.rerun()
        else:
            st.error(f"⚠️ ¿Eliminar '{p_s}'?"); 
            if st.button("🗑️ SÍ, ELIMINAR"):
                tabla_stock.delete_item(Key={'TenantID': st.session_state.tenant, 'Producto': p_s})
                registrar_kardex(p_s, 0, "ELIMINADO"); st.warning("Producto borrado."); import time; time.sleep(1); st.rerun()
