...
with t1:
    if st.session_state.boleta:
        ...
        ct = c2.number_input("Cant:", min_value=1, value=1, key=f"cv_{p_s}")
        if p_s:
            # CORRECCIÓN: Acceso seguro a la fila
            ...
            if st.session_state.confirmar:
                ...
                if cc1.button("✅ SÍ"):
                    ...
                    for i, it in enumerate(st.session_state.carrito):
                        ...
                        s_act = int(df_inv[df_inv['Producto']==it['Producto']]['Stock'].iloc[0])
...
with t3:
    ...
    if v_d:
        ...
        c1.metric("💵 EFECTIVO", f"S/ {e_t:g}", f"Ganancia: S/ {e_g:g}")
        c2.metric("🟣 YAPE", f"S/ {y_t:g}", f"Ganancia: S/ {y_g:g}")
        c3.metric("🔵 PLIN", f"S/ {p_t:g}", f"Ganancia: S/ {p_g:g}")
...
with t5:
    ...
    with st.form("fn"):
        ...
        if st.form_submit_button("🚀 GUARDAR"):
            ...
            registrar_kardex(pn, sn, "ENTRADA/NUEVO")
            st.success(f"✅ Guardado"); import time; time.sleep(1); st.rerun()

with t6:
    st.subheader("🛠️ Gestión")
    op = st.radio("Acción:", ["➕ REPONER", "🗑️ ELIMINAR"], horizontal=True)
    ...
    if p_m:
        ...
        r_m = df_inv[df_inv['Producto'] == p_s].iloc[0]
        if op == "➕ REPONER":
            c_m = st.number_input("¿Cuánto entra?", min_value=1, key=f"cant_m_{p_s}") 
            if st.button("✅ ACTUALIZAR STOCK"):
                ...
                registrar_kardex(p_s, c_m, f"REPOSICIÓN (+{c_m})")
